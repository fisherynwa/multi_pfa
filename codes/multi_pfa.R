...
multi_pfa <- function(X, y, tval = list(), reg = "L1", K = list(), m = 0.95){
    ...
    if(is.vector(y) && length(unique(y)) > 5){
        stop("The recent version of the package is limitied up to five classes.")}
    
    if(is.matrix(y) && dim(y)[2] < 3){
        stop("The outcome variable should be multicategorical!")}
    
    if(is.matrix(y) && dim(y)[2] > 5){
        stop("The recent version of the package is limitied up to five classes.")}
    ...
    if((is.vector(y) && length(unique(y)) == 4) ||
       (is.matrix(y) && dim(y)[2] == 4)){
        ...
        if(!is.empty.list(K)){  k1 = K[[1]]; k2 = K[[2]]; k3 = K[[3]]} else {
            k1 <- rlang::missing_arg(); k2 <- rlang::missing_arg()
    ...
    if((is.vector(y) && length(unique(y)) == 5) ||
       (is.matrix(y) && dim(y)[2] == 5)){
        ...
        if(is.list(t)){  t1 = t[[1]]; t2 = t[[2]]; t3 = t[[3]]; t4 = t[[4]]}
        
        if(is.list(K)){  k1 = K[[1]]; k2 = K[[2]]; k3 = K[[3]]; k4 = K[[4]]}
